...
getProductData().then(data => {
  ...
  // ✅ 3. Attach event listeners AFTER product cards are in the DOM
...
function generateProductCard(product) {
  ...
  return `<div class="${columnClass} col-6 main-product">
      <div class="product-box position-relative">
        <div class="position-absolute rounded-circle product_wishlist cursor-pointer text-center" data-id="${product.id}">
        ...
        <div class="font-size12 pt-2">
          ...
          <span class="font-weight500">4.67</span>
          <span class="color-Blackgray">(8 reviews)</span>
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Only fetch product details if a product ID is present in the URL
  if (productId) {
    getProductData().then(data => {
      const product = [...data.products, ...data.featured_products].find(p => p.id == productId); // Search in both
      if (product) {
        renderProductDetails(product);
        setupDetailPageCart(product);
      } else {
        console.warn(`Product with ID ${productId} not found.`);
        // Optionally redirect to a 404 page or show a "product not found" message
      }
    });
  }
...
function renderPaginationButtons(totalItems) {
  ...
  if (currentPage > 1) {
    ...
    prevBtn.addEventListener("click", () => {
      ...
      document.getElementById("all-product-list").scrollIntoView({ behavior: "smooth" });
    });
...
function renderWishlist() {
  ...
  attachRemoveFromWishlist();
  attachCartListeners(); // reuse your global add-to-cart logic
}
...
function attachWishlistListeners() {
  document.querySelectorAll(".product_wishlist").forEach(button => {
    button.addEventListener("click", (event) => {
      event.stopPropagation(); // Prevent card click if it's a nested element
      const productId = button.dataset.id;
...
function toggleWishlist(productId) {
  ...
  if (productIndex !== -1) {
    ...
    // Find the product data from allAvailableProducts (assuming it's loaded)
    const productData = window.allAvailableProducts.find(p => String(p.id) === productId);
    if (productData) {
  ...
  updateWishlistCount(); // This will now update the count whenever an item is added or removed
}
